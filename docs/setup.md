...
- Python 3.12+
- pip
- PostgreSQL database for `ppl-hr` modules
...
- `DATABASE_URL` - SQLAlchemy PostgreSQL URL for `ppl-hr`.
...
If no PostgreSQL URL is set, the app falls back to local SQLite for lightweight development and tests.
