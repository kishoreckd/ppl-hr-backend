...
- `implementation-plan/` - phased backend implementation plan for the full `ppl-hr` product scope.

## Notes

`ppl-hr` modules are implemented with PostgreSQL/SQLAlchemy 2.x and Alembic. The original organizational chart feature remains on the existing MongoDB/Motor route layer so the prior chart structure is preserved while the HRMS backend grows around it.
